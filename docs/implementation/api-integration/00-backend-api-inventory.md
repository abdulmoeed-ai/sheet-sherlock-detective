...
- Keep these backend review-cell endpoints for now because manager submission still depends on review-cell status resolution.
- Do not expose a separate Diff Review frontend route; reuse these endpoints only from diagnosis/workflow screens that need to resolve review-cell blockers.
