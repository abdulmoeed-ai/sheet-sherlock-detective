...
export function getAskAiPromptKeyAction(event: {
  key: string;
  shiftKey: boolean;
}): AskAiPromptKeyAction {
  if (event.key !== "Enter") return "ignore";
  return event.shiftKey ? "newline" : "submit";
}

export function getAskAiPromptTextareaLayout(scrollHeight: number): {
  height: number;
  overflowY: AskAiPromptOverflow;
} {
  const height = Math.max(
    ASK_AI_PROMPT_MIN_HEIGHT,
    Math.min(scrollHeight, ASK_AI_PROMPT_MAX_HEIGHT),
  );
  return {
    height,
